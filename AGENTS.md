...
| 17 | **Adopt, don't invent** | Prefer existing OSS libraries and standards over custom implementations. Nexus's value is in orchestration and intelligence — cross-source synthesis, priority scoring, delegation tracking, briefing assembly — not in reinventing solved problems. Use CalDAV for calendar/tasks, FTS5 for search, croniter for scheduling, pdfplumber for parsing, matplotlib for charts, existing MCP servers where available. Build only what no OSS project provides. |
| 18 | **Local-first, sync-optional** | Every feature must work with zero external accounts. Local SQLite is the default storage backend. External services (Google, Microsoft, CalDAV) are optional sync targets that enhance the experience but are never required. Self-hosted first. |
