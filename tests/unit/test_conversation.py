...
class TestSetters:
    ...
    def test_set_content_store(self) -> None:
        ...
        assert c._content_store is s
        assert c._dashboard_config is d
...
class TestBuildCapabilities:
    def test_with_mcp_tools(self) -> None:
        c = _make_conv()
        c._mcp = MagicMock()
        c._mcp.all_tool_schemas.return_value = [{"name": "t1"}, {"name": "t2"}]
        result = c._build_capabilities_section()
...
class TestToolUseLoop:
    async def test_no_llm(self) -> None:
        ...
        c._llm = None
        tenant = TenantContext(tenant_id="t1", name="A")
        result = await c._tool_use_loop([], "", None, tenant)
    ...
    async def test_no_tool_calls(self) -> None:
        ...
        tenant = TenantContext(tenant_id="t1", name="A")
        result = await c._tool_use_loop([{"role": "user", "content": "hi"}], "", None, tenant)
    ...
    async def test_with_tool_calls(self) -> None:
        ...
        c._llm.chat.side_effect = [first_resp, second_resp]

        c._mcp = AsyncMock()
        c._mcp.call_tool.return_value = "result data"
        c._policy = MagicMock()
        c._policy.check.return_value = PolicyDecision.ALLOW
        c._trust = MagicMock()
        c._trust.get_score.return_value = 0.8

        tenant = TenantContext(tenant_id="t1", name="A")
        result = await c._tool_use_loop(
...
class TestExecuteToolWithGovernance:
    async def test_deny(self) -> None:
        ...
        c._trust = MagicMock()
        c._trust.get_score.return_value = 0.3
        tenant = TenantContext(tenant_id="t1", name="A")
        result = await c._execute_tool_with_governance("delete_email", {}, tenant)
    ...
    async def test_allow(self) -> None:
        ...
        c._policy.check.return_value = PolicyDecision.ALLOW
        c._trust = MagicMock()
        c._trust.get_score.return_value = 0.9
        c._mcp = AsyncMock()
        c._mcp.call_tool.return_value = "done"
        tenant = TenantContext(tenant_id="t1", name="A")
        result = await c._execute_tool_with_governance("search_gmail", {}, tenant)
        assert result == "done"
...
class TestHandleInbound:
    ...
    async def test_llm_failure(self) -> None:
        ...
        sent_text = c._transport.send_text.call_args[0][1]
...
class TestSendReply:
    async def test_with_transport(self) -> None:
        ...
        c._transport.send_text.assert_called_once_with("c1", "hello")
...
class TestSendResponseWithViewer:
    async def test_short_response(self) -> None:
        c = _make_conv()
        c._transport = AsyncMock()
        await c._send_response_with_viewer("c1", "short text")
        c._transport.send_text.assert_called_once_with("c1", "short text")

    async def test_long_response_with_store(self) -> None:
        c = _make_conv()
        c._transport = AsyncMock()
        c._content_store = MagicMock()
        c._content_store.store.return_value = "abc123"
        c._dashboard_config = MagicMock()
        c._dashboard_config.host = "localhost"
        c._dashboard_config.port = 8080
        long_text = "x" * 3000
        await c._send_response_with_viewer("c1", long_text)
        sent = c._transport.send_text.call_args[0][1]
...
class TestSendTyping:
    async def test_with_transport(self) -> None:
        ...
        c._transport.send_typing.assert_called_once()
